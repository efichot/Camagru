<?php
	...
	function sendInformativeMail($login, $email) {
		...
		$subject = 'Camagru - Modifier votre mot de passe';
		$message = '<html><head><title>Camagru - Une de vos photo a reçu un commentaire.</title></head>
		<body>
			<h4>Bonjour, ' . $login . '.</h4>
			<p>Une de vos photo a reçu un commentaire. Connectez vous vite ici <a href="' . $validateURL . '" target="_blank">ici</a>.</p>
		</body></html>';
		$header = 'MIME-Version: 1.0' . '\r\n';
		$header .= 'Content-type: text/html; charset=iso-8859-1' . '\r\n';
		$header .= 'To: ' . $login . ' <' . $email . '>' . '\r\n';
